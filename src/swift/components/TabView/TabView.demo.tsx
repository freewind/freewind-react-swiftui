...
export const TabViewDemo: FC = () => {
  const primarySelection = useBinding<'contacts' | 'images' | 'files'>('contacts')
  const secondarySelection = useBinding<'contacts' | 'images' | 'files'>('contacts')

  return (
    <VStack spacing={18} frame={{ maxWidth: 'infinity', alignment: 'leading' }}>
      <FormSection title="基础页签容器">
        <TabView selection={primarySelection} tint="green">
      ...
      <PlaygroundSection
        ...
        preview={
          <TabView selection={secondarySelection} tabBarHidden={secondarySelection.value === 'images'}>
            <Tab tag="contacts" title="聊天" systemImage="iphone">
              <VStack spacing={8} alignment="leading">
                <Text>当前 tab: 聊天</Text>
                <Text foregroundStyle="secondary">selection: {secondarySelection.value}</Text>
              </VStack>
            </Tab>
